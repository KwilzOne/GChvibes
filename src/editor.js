...
const fs = require('fs'),
		...
		remapper = require('./utils/remapper'),
		layouts = require('./libs/layouts'),
		keycodes = require('./libs/keycodes'),
		$ = require('./assets/jquery'),
		layout = layouts[process.platform],
		{ sizes } = layouts,
		os_keycode = keycodes[process.platform],
		CUSTOM_PACKS_DIR = path.join(__dirname, '../../../custom')

let selected_keycode = null,
	current_edit_mode = 'visual',
	current_key_define_mode = 'single',
	pack_data = {
...
Object.keys(pack_data.defines).map(kc => { pack_data.defines[kc] = null })
