...
async function loadPacks(status_display_elem, app_body) {
  ...
  folders.map((folder) => {
    ...
    const is_custom = folder.indexOf('GChvibes') > -1 ? true : false;
...
function checkIfAllSoundLoaded(status_display_elem, app_body) {
  ...
  status_display_elem.innerHTML = 'GChvibes';
...
(function (window, document) {
  window.addEventListener('DOMContentLoaded', async () => {
    ...
    const no_update_available = document.getElementById('no-update-available');
    ...
    fetch('https://api.github.com/repos/KwilzOne/GChvibes/releases/latest')
      .then((res) => res.json())
      .then((json) => {
        if (json.tag_name > APP_VERSION) {
          new_version.innerHTML = json.tag_name;
					no_update_available.classList.add('hidden');
